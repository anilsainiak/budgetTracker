...
const columns: ColumnDef<TransactionHistoryRow>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => (
            <div>
                <div className={cn('capitalize rounded-lg text-center p-2', row.original.type === 'income' ? 'bg-emerald-400/10 text-emerald-500' : 'bg-red-400/10 text-red-500')}>{row.original.type}</div>
            </div>
        )
    },
    {
...
const TransactionTable = ({ from, to }: Props) => {
    ...
    const handleExportsCSV = (data:any[])=>{
        if(data.length===0){
        ...
        <div className="w-full">
            <div className="flex flex-wrap items-end justify-between gap-2 py-4">
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    onClick={()=>{
                        const data = table.getFilteredRowModel().rows.map((row:Row<TransactionHistoryRow>)=>({
